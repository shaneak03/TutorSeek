...
describe('Supabase API Post Functions', () => {
  ...
  describe('postUserProfile', () => {
    ...
    it('should successfully post user profile', async () => {
      const mockData = { id: 'user123' };
      const mockInsert = jest.fn().mockResolvedValue({ data: mockData, error: null });
      (supabase.from as jest.Mock).mockReturnValue({ insert: mockInsert });

      const result = await postUserProfile(mockUserProfile);

      expect(supabase.from).toHaveBeenCalledWith('users');
      expect(mockInsert).toHaveBeenCalledWith(mockUserProfile);
      expect(result).toEqual(mockData);
    });

    it('should handle error when posting user profile', async () => {
      const mockError = new Error('Database error');
      const mockInsert = jest.fn().mockResolvedValue({ data: null, error: mockError });
      (supabase.from as jest.Mock).mockReturnValue({ insert: mockInsert });

      await expect(postUserProfile(mockUserProfile)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error posting user profile:', mockError);
    });
  });

  describe('postTutorProfile', () => {
    ...
    it('should successfully post tutor profile', async () => {
      const mockData = { id: 'tutor123' };
      const mockInsert = jest.fn().mockResolvedValue({ data: mockData, error: null });
      (supabase.from as jest.Mock).mockReturnValue({ insert: mockInsert });

      const result = await postTutorProfile(mockTutorProfile);

      expect(supabase.from).toHaveBeenCalledWith('tutors');
      expect(mockInsert).toHaveBeenCalledWith(mockTutorProfile);
      expect(result).toEqual(mockData);
    });

    it('should handle error when posting tutor profile', async () => {
      const mockError = new Error('Database error');
      const mockInsert = jest.fn().mockResolvedValue({ data: null, error: mockError });
      (supabase.from as jest.Mock).mockReturnValue({ insert: mockInsert });

      await expect(postTutorProfile(mockTutorProfile)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error posting tutor profile:', mockError);
    });
  });

  describe('postStudentProfile', () => {
    const mockStudentProfile: StudentProfile = {
      id: 'student123',
    };

    it('should successfully post student profile', async () => {
      const mockData = { id: 'student123' };
      const mockInsert = jest.fn().mockResolvedValue({ data: mockData, error: null });
      (supabase.from as jest.Mock).mockReturnValue({ insert: mockInsert });

      const result = await postStudentProfile(mockStudentProfile);

      expect(supabase.from).toHaveBeenCalledWith('students');
      expect(mockInsert).toHaveBeenCalledWith(mockStudentProfile);
      expect(result).toEqual(mockData);
    });

    it('should handle error when posting student profile', async () => {
      const mockError = new Error('Database error');
      const mockInsert = jest.fn().mockResolvedValue({ data: null, error: mockError });
      (supabase.from as jest.Mock).mockReturnValue({ insert: mockInsert });

      await expect(postStudentProfile(mockStudentProfile)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error posting student profile:', mockError);
    });
  });
  ...
  describe('postReview', () => {
    ...
    it('should handle error when posting review', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Error posting review:', mockError);
    });
  });

  describe('postTutorSubject', () => {
    const subjectData = {
      tutor_id: 'tutor123',
      subject_id: 1,
      level_id: 2,
    ...
    it('should successfully post tutor subject', async () => {
      const mockData = { id: 'subject123' };
      const mockInsert = jest.fn().mockResolvedValue({ data: mockData, error: null });
      (supabase.from as jest.Mock).mockReturnValue({ insert: mockInsert });

      const result = await postTutorSubject(
        subjectData.tutor_id,
        subjectData.subject_id,
        subjectData.level_id
      );

      expect(supabase.from).toHaveBeenCalledWith('tutor_subjects');
      expect(mockInsert).toHaveBeenCalledWith(subjectData);
      expect(result).toEqual(mockData);
    });

    it('should handle error when posting tutor subject', async () => {
      const mockError = new Error('Database error');
      const mockInsert = jest.fn().mockResolvedValue({ data: null, error: mockError });
      (supabase.from as jest.Mock).mockReturnValue({ insert: mockInsert });
      ...
      expect(console.error).toHaveBeenCalledWith('Error posting tutor subject:', mockError);
    });
  });
  ...
  describe('createTimeTable', () => {
    ...
    it('should handle error when counting timeslots', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('Error creating timetable' + mockError);
    });
  });
  ...
  describe('updateLastSeen', () => {
    ...
    it('should successfully update last seen timestamp', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(`Updated last_online_at for user ${userId}`);
    });
    ...
    it('should handle error when updating last seen', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Failed to update last_online_at:', mockError);
    });
  });
  ...
  describe('updateUserProfile', () => {
    ...
    it('should handle error when updating user profile', async () => {
      ...
      await expect(updateUserProfile(mockUserProfile)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error updating user profile:', mockError);
    });
  });
  ...
  describe('updateRatingReview', () => {
    ...
    it('should handle error when updating rating and review', async () => {
      ...
      await expect(updateRatingReview(avgRating, reviewCount, tutorId)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error updating rating and review count', mockError);
    });
  });
  ...
  describe('updateTutorProfile', () => {
    ...
    it('should handle error when updating tutor profile', async () => {
      ...
      await expect(updateTutorProfile(mockTutorProfileData)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error updating tutor profile:', mockError);
    });
  });
  ...
  describe('addTutorSubjects', () => {
    ...
    it('should handle error when adding tutor subjects', async () => {
      ...
      await expect(addTutorSubjects(subjects, tutorId)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error adding tutor subjects', mockError);
    });
  });
  ...
  describe('deleteTutorSubjects', () => {
    ...
    it('should handle error when deleting tutor subjects', async () => {
      ...
      await expect(deleteTutorSubjects(ids, tutorId)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error removing tutor subjects', mockError);
    });
  });
  ...
  describe('updateStudentProfile', () => {
    ...
    it('should successfully update student profile', async () => {
      ...
      const result = await updateStudentProfile(mockStudentProfile);

      expect(supabase.from).toHaveBeenCalledWith('students');
      expect(mockUpdate).toHaveBeenCalledWith(mockStudentProfile);
      expect(result).toEqual(mockData);
    });

    it('should handle error when updating student profile', async () => {
      const mockError = new Error('Database error');
      const mockUpdate = jest.fn().mockReturnValue({
        eq: jest.fn().mockResolvedValue({ data: null, error: mockError }),
      });
      ...
      await expect(updateStudentProfile(mockStudentProfile)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error updating student profile:', mockError);
    });
  });
  ...
  describe('markMessagesAsRead', () => {
    ...
    it('should handle error when chat not found', async () => {
      ...
      await expect(markMessagesAsRead(chatId, userId)).rejects.toThrow('Chat not found');
      expect(console.error).toHaveBeenCalledWith('Error marking messages as read:', expect.any(Error));
    });
    ...
    it('should handle database error when fetching chat', async () => {
      ...
      await expect(markMessagesAsRead(chatId, userId)).rejects.toThrow('Database error');
  ...
  describe('postChatMessage', () => {
    ...
    it('should post message to existing chat', async () => {
      ...
      const result = await postChatMessage(mockMessageData, senderRole, tutorId, studentId);
    ...
    it('should handle error when posting chat message', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Error posting chat message:', mockError);
    });
  });
});