...
describe("Supabase API Get Functions", () => {
  ...
  const createMockChatWithParticipants = (
    id: number,
    tutorId: string,
    studentId: string
  ): ChatWithParticipants => ({
  ...
  describe("getChatsByUserId", () => {
    it("should fetch chats for a user and return ChatWithParticipants[]", async () => {
      ...
      expect(mockChatsQuery.select).toHaveBeenCalledWith(
      ...
      expect(mockChatsQuery.or).toHaveBeenCalledWith(
        `tutor_id.eq.${userId},student_id.eq.${userId}`
      );
      expect(mockChatsQuery.order).toHaveBeenCalledWith("updated_at", {
        ascending: false,
      });

      expect(supabase.from).toHaveBeenCalledWith("messages");
      expect(mockMessagesQuery.select).toHaveBeenCalledWith(
        "id, content, created_at, sender_id"
      );
      expect(mockMessagesQuery.eq).toHaveBeenCalledWith("chat_id", mockChat.id);
      expect(mockMessagesQuery.order).toHaveBeenCalledWith("created_at", {
        ascending: false,
      });
  ...
  describe("getTutors", () => {
    ...
    it("should return empty array on error", async () => {
      const mockTutorsQuery = {
        ...
        order: jest
          .fn()
          .mockResolvedValue({ data: [], error: new Error("Failed") }),
      };
  ...
  describe("getSubjectsByTutorId", () => {
    ...
    it("should throw error when query fails", async () => {
      ...
      await expect(getSubjectsByTutorId("tutor1")).rejects.toThrow(
  ...
  describe("getAvgRatingAndReviewCount", () => {
    ...
    it("should throw error when RPC fails", async () => {
      ...
      await expect(getAvgRatingAndReviewCount("tutor1")).rejects.toThrow(
        "RPC failed"
      );
    });
  });
